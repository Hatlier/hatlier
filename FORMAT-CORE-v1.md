...
6. **保存形式は HTML だが、フォーマットの正本は埋め込み Hatlier データである**
...
11. **出口（profile）は第一級** — ホスト先の厳しさに合わせ、部品の `needs` でパレットを制限できる（§8）
...
| `target` | `doc.layout.target`（現行 flat: `doc.target`） | 主な書き出し: `web` / `zip` / `png` / `pdf` / `slide`。編集ガイド用。0.6.2 |
| `artboard` | `doc.layout.artboard`（現行 flat） | PNG 時のサイズ感。0.6.2 |
...
Format Core は **形の契約**。プロファイルは **どの能力を許すか（出口契約）**。  
これは付録ではない。**Web にホストするユーザにとって第一級の要件**である。

### 8.1 なぜ必要か

書き出しは主に **1 HTML** または **ZIP** になる。ホスト先の CSP（Content Security Policy）はまちまちである。

| 公開先の厳しさ | 起きうること |
|---|---|
| CSP が強い（インライン／外部 JS 禁止） | カルーセルのループ、ページ内検索、live 時計などが **動かない／CSP 違反** |
| ふつうの静的ホスト | 軽量 JS は動くことが多い |
| アプリ的ホスト | fetch 等も許されることがある |

「作ってから公開先で死ぬ」を防ぐため、**編集の段階で出口を選ばせ、部品パレットをそれに合わせて制限する**。

### 8.2 プロファイル（現行）

| ID | 呼び方 | 許可する `needs`（概要） |
|---|---|---|
| `safe`（**既定**） | いちばん無難 | なし（HTML+CSS のみ。**JS なし**） |
| `standard` | ややゆるめ | `js`, `media` |
| `rich` | 拡張（Later） | `js`, `media`, `fetch` 等 |

詳細の UX・文言は [`FORMAT-v2.md`](./FORMAT-v2.md) §3。Core が固定するのは次の不変条件である。

### 8.3 出口に関する不変条件

1. **文書は必ず `profile` を持つ**（省略時は `safe`）
2. **部品は `needs` を宣言できる**（省略＝`[]`＝全プロファイル可）
3. **パレットは現在の `profile` で使えない部品を出さない**（または理由付きで出せない）
4. **厳しく切り替えるとき、非対応部品は黙って消さない** — 一覧して確認する
5. **書き出し HTML に `data-profile` を付ける**（再編集・監査の手がかり）
6. **見た目の `variant` は出口と独立** — 枠デザイン等は CSS のみなら `safe` でも使える
7. Core の unknown preservation は **どの profile でも守る**

### 8.4 Format Core との役割分担

| | Format Core | Profile（出口） |
|---|---|---|
| 問うこと | どう保存し、壊さないか | どこに出しても困らないか |
| 例 | `schema`, unknown 保持 | `needs:["js"]` を `safe` で禁ずる |
| 混同しない | 形 | 能力 |

**テイスト（variant）は出口を広げない。** テキスト枠・区切り線の見た目切替は CSS のみなので `safe` でも全開放してよい。JS を要する部品だけが出口で絞られる。

### 8.5 実装の現状（参照）

エディタは `PROFILES` / `needsOf` / `allowedBlockTypes` / デザインパネルの「出力の向き」で上記を実装済み（0.4.0〜）。  
Format Core はこれを **捨ててはならない契約**として明文化する。
...
| 2026-07-27 | §8 を「出口は第一級」に拡充（CSP / profile / needs）。variant（見た目）は出口と独立と明記 |
